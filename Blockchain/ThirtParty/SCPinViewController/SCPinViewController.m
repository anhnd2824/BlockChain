...
- (void)viewDidLoad {
    ...
    self.numberWrong = 0;
    //Init scope edit
    if (self.scope == SCPinViewControllerScopeEdit) {
        self.editScope = SCPinViewControllerScopeEditViewValidate;
    }
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if ([CommonUtils isTouchIDAvailable] && self.scope == SCPinViewControllerScopeValidate) {
...
//            }
    ...
    if (self.scope != SCPinViewControllerScopeValidate) {
        [self.touchIDButton setHidden:YES];
    }
}
...
-(void)createPinView {
    ...
    switch (self.scope) {
        ...
        case SCPinViewControllerScopeEdit: {
            NSString *pin = [self.dataSource codeForPinViewController:self];
            length = [pin length];
            break;
        }
...
-(void)setCurrentPin:(NSString *)currentPin {
    ...
    switch (self.scope) {
        case SCPinViewControllerScopeValidate:{
            ...
            if (equalLength) {
                [self validatePin:currentPin scope:self.scope];
            }
            break;
        }
        case SCPinViewControllerScopeCreate: {
        ...
        case SCPinViewControllerScopeEdit:{
            BOOL equalLength = ([currentPin length] == [[self.dataSource codeForPinViewController:self] length]);
            if (equalLength) {
                if (self.editScope == SCPinViewControllerScopeEditViewValidate) {
                    [self validatePin:currentPin scope:self.scope];
                }else {
                    [self.createDelegate pinViewController:self didSetNewPin:currentPin];
                }
...
-(void)validatePin:(NSString *)pin scope:(SCPinViewControllerScope) scope{
    if ([[self.dataSource codeForPinViewController:self] isEqualToString:pin]) {
        [self correctPin:scope];
    } else {
        [self wrongPin];
    }
}

-(void)correctPin:(SCPinViewControllerScope) scope{
    if (scope == SCPinViewControllerScopeValidate) {
        ...
        self.numberWrong = 0;
        self.titleLabel.text = _appearance.titleText = @"Enter new password";
        self.editScope = SCPinViewControllerScopeEditViewCreate;
        [self configureView];
        [self setupInitialState];
        [self clearCurrentPin];
        [self createPinView];
...
- (void)wrongPin {
    ...
    dispatch_after(delayInSeconds, dispatch_get_main_queue(), ^(void){
        ...
        strongSelf.numberWrong += 1;
        AudioServicesPlaySystemSound(kSystemSoundID_Vibrate);
        CAAnimation * shake = [self makeShakeAnimation];
        [strongSelf.viewForPins.layer addAnimation:shake forKey:@"shake"];
        if ([strongSelf.validateDelegate respondsToSelector:@selector(pinViewControllerDidSetWrongPin:numberWrong:)]) {
            [strongSelf.validateDelegate pinViewControllerDidSetWrongPin:strongSelf numberWrong:strongSelf.numberWrong];
        }
